...
def generate(states_doc, byid):
    ...
    out.append({
        ...
        "ui": [
            ...
            ["chips", ["That's about right", "Change it"]],
        ...
        "spec": {
            ...
            "logic": [
                ...
                "Confirm or correct (Vatsal, 11 Sep 2026): each row carries last quarter's value with its source tag; That's about right stores it again with the tag; Change it reveals the input for that row. The derivation reads carried from last quarter, never an estimate.",
                "Mixed sources: a locked manual field is not overwritten by a later Account Aggregator fetch (spec v0.1 A08, kept); the row shows the manual lock.",
            ...
            "states": ["S16 AA not connected: this is the review", "Prefilled: every row shows last quarter's value with its tag; the primary action per row is That's about right", "Mixed sources: AA-fed rows show the fetched value and the manual lock; only typed rows are asked"],
        ...
        "v02": {"status": "new", "causes": [CAUSE, CAUSE_P9]}, "after": "Q04",
    })
    return out
